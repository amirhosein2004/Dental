document.addEventListener('DOMContentLoaded', function () {
    ...
    const noMoreGalleries = document.getElementById('no-more-galleries');
        ...
        loadMoreButton.addEventListener('click', function () {
            ...
            .then(data => {
                ...
                data.galleries.forEach(gallery => {
                    const galleryHtml = createGalleryItem(gallery);
                    galleryContainer.insertAdjacentHTML('beforeend', galleryHtml);
                });

                const newOffset = offset + data.galleries.length;
                loadMoreButton.setAttribute('data-offset', newOffset);

                if (!data.has_more) {
                    loadMoreButton.style.display = 'none';
                    noMoreGalleries.style.display = 'block';
                }

                initializeSwipers();
                initializeBootstrapEvents();
            })
            .catch(error => console.error('خطا در لود گالری‌ها:', error));
        });
    }
...
// تابع برای ساخت HTML هر گالری
function createGalleryItem(gallery) {
    let imagesHtml = '';
    gallery.image.forEach(imgUrl => {
        imagesHtml += `
...
// تابع برای مقداردهی رویدادهای بوت‌استرپ
