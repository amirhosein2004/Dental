document.addEventListener("DOMContentLoaded", function () {
    ...
    let filterForm = document.getElementById("blogFilterForm");
    let noMoreBlogs = document.getElementById("no-more-blogs");

    if (loadMoreButton) {
        loadMoreButton.addEventListener("click", function () {
            let offset = parseInt(this.getAttribute("data-offset")) || 0;

            // جمع‌آوری داده‌های فرم فیلتر
            let formData = new FormData(filterForm);
            let queryParams = new URLSearchParams(formData);
            queryParams.set("offset", offset);

            fetch(`${loadMoreButton.getAttribute("data-url")}?${queryParams.toString()}`, {
                method: "GET",
                headers: {
            ...
            .then(response => {
                if (!response.ok) {
                    console.error("خطا در پاسخ سرور:", response.status);
            ...
            .then(data => {
                ...
                if (data.blogs && data.blogs.length > 0) {
                    data.blogs.forEach(blog => {
                        ...
                        div.classList.add("col-12", "col-sm-6", "col-md-4", "col-lg-4");
                                    ...
                                    <div class="card-body p-4 d-flex flex-column gap-3">
                                        <div class="categories mb-2 d-flex flex-wrap gap-2">
                                            ${blog.categories.length > 0 ? blog.categories.map(category => `
                                                <span class="badge badge-category" itemprop="keywords">${category}</span>
                                            `).join('') : '<span class="badge badge-category text-muted">بدون دسته‌بندی</span>'}
                                        </div>
                    ...
                    loadMoreButton.setAttribute("data-offset", offset + data.blogs.length);

                    if (noMoreBlogs) {
                        noMoreBlogs.style.display = "none";
                    }
                } else {
                    console.log("هیچ بلاگی برای نمایش نیست.");
                    if (noMoreBlogs) {
